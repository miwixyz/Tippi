...
Tippi/
  ...
  Core/                  ← business logic (DictationController, HotkeyManager, KeyCombo)
    Snippets/            ← system-wide typing expansion (Espanso-compatible, v2.0)
    Emoji/               ← emoji database, search, `:name:` inline matching (v2.1)
    SelectionPopup/      ← PopClip-style bar shown at a text selection (v2.0)
...
- **A floating panel may only become key window if it owns a real text field.** macOS has exactly one key window system-wide; taking it means the app the user is actually typing in stops receiving ⌘C/⌘V/⌘X, Delete, plain typing and Escape. This shipped as a showstopper in v2.0.0 (fixed in v2.0.1): the selection action bar copied the `NonActivatingKeyPanel` pattern from the translate window, but it is buttons only and never needed keyboard focus. Decide per panel, don't copy the pattern:
  - **Buttons only** → `canBecomeKey = false`, first click still works via `acceptsFirstMouse`, handle Escape with a dedicated global monitor (`SelectionActionBarPanel`).
  - **Has a text field** → `canBecomeKey = true` is required and correct (`TranslateQuickPanel`, `PromptPopupController`, `EmojiPickerPanel`).
- **Arrow keys and Return inside a panel with a search field belong in a *local* `NSEvent` monitor, not SwiftUI `.onKeyPress`** — a focused `TextField` swallows ↑/↓ to move its insertion point. A local monitor only sees events already routed to Tippi, so unlike a CGEvent tap it can never affect another app (`EmojiPickerPanel.installKeyMonitor`).
- **Only ever run one system-wide keystroke watcher.** `SnippetKeystrokeMonitor` serves three expansion paths — snippets, `:name:` emoji, and emoticons — checked in that order (most explicit wins). Start/stop it exclusively through `AppDelegate.applyKeystrokeMonitorState()`, which ORs all three feature toggles; forgetting one there means that feature silently never fires. A second global monitor would double every keystroke into the matcher. The one deliberate exception is the Labs autocomplete's *active* tap (`AutocompleteController`): it has to swallow ⇥, which a monitor cannot, feeds nothing into the matcher, and exists only while that feature is on.
- **`@Published` fires in `willSet`** — inside a `sink` the stored property still holds the *old* value. Use the value handed to the closure, never re-read the source (this bit the snippet/emoji monitor wiring).
