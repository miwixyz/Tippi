...
| v2.2.0  | ✅ Done | **Dictation on a single key** — tap the key to toggle, hold it to record only while held (right Shift by default, any modifier). The key combination stays the default, so no configured hot key changes. A hold that never sees a release stops itself after five minutes. Plus a **running timer** in the recording pill, a **top/bottom position** for it, and **Liquid Glass** on every floating surface on macOS 26+ (unchanged below — deployment target stays macOS 15). Settings now says when Input Monitoring is missing instead of the hot key silently doing nothing |
