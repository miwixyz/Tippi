...
- **Autocomplete while typing (Labs, off by default, v2.15)** — pause briefly while typing and your **local** model suggests the rest of the sentence (up to eight words) in grey right at the cursor, in any app with a regular text field and in Apple Mail. **⇥ takes the next word**, the rest stays; keep typing the suggested letters and it stays too, anything else dismisses it. Your custom words are passed along so names and brands come out spelled your way. Tab is only intercepted while a suggestion is visible, so indenting in a code editor keeps working. Everything you type goes **only to the MLX server Tippi started itself** on this Mac (never a cloud provider, never a foreign server that happens to sit on the same port) and is **never stored or logged**. Password fields, secure input and an editable app list (password managers and terminals by default) are never read. Turn it on from the menu bar or in Settings → General. Design: `docs/SECURE-DESIGN-autocomplete.md`
...
- **Dark Mode + Light Mode** — fully adaptive UI; all surfaces use macOS semantic materials and system colors; brand palette has explicit dark-mode variants. Follows the system by default, or pick one for Tippi alone in Settings → General → **Appearance (System · Light · Dark)** — applies to every window, panel and popup, stays on this Mac (not synced: the right choice depends on the display)
- **DE + EN UI**
...
Settings → Providers → "Default Provider" picker. Tippi tries the chosen provider first. If it has no key, it falls through to the next configured one. Each provider also has a "Model" field — leave blank for the default (recommended in Sep 2026: `gpt-6-luna`, `claude-haiku-4-5`, `gemini-flash-latest`, `mistral-small-latest`, `llama3.3`).
...
|-----------|------------|---------|---------|-------|
| OpenAI    | $          | Fast    | ★★★★   | Most popular. `gpt-6-luna` is the cheapest current model; Tippi turns its reasoning off. |
...
- **Autocomplete (Labs) fully local** — text before the cursor goes only to the MLX server Tippi started itself, over `127.0.0.1`; nothing is stored, the log holds only app, lengths and duration
...
|---------|--------|------------|
| v2.15.0 | ✅ Done | **Autocomplete while typing (Labs, off by default)** — a Cotypist-style suggestion of the rest of the sentence from the local MLX model, grey at the cursor; ⇥ word by word, keep typing to keep it, works in Apple Mail (WebKit text markers). Local only: just the MLX server Tippi started, over `127.0.0.1`, nothing stored; password fields, secure input, password managers and terminals are never read. **Dictation mode survives updates** — tests no longer write to the installed app's real settings, and `make test` now guards that. Custom words at a readable size. |
| v2.14.0 | ✅ Done | **Pronunciation variants, appearance switch, Notes from the Dock** — custom words accept `Tipi → Tippi`: a fixed whole-word swap after transcription (Parakeet + Whisper), before AI cleanup, so it works with cleanup off too; the strict AI rule that never rewrites real words stays. Settings → General → **Appearance (System · Light · Dark)** for the whole app, per Mac. Clicking the pinned Dock icon opens Notes. |
| v2.13.0 | ✅ Done | **GPT-6 Luna as the OpenAI default, with reasoning switched off** — half the price of gpt-5.6-luna; Tippi sends `reasoning_effort: "none"` for GPT-6 Luna and Sol (without it they think at "medium" on every call and OpenAI rejects the temperature with HTTP 400). Claude Opus 5.5 as the Anthropic premium pick. Two OpenRouter ids fixed (`~google/gemini-flash-latest`, `anthropic/claude-haiku-4.5`) with migration for stored picks. SwiftLint as a release gate. |
| v2.12.5 | ✅ Done | **Update window no longer hides behind other apps** — a scheduled check now becomes a gentle reminder (notification + "Update to … ready" in the menu) instead of a window macOS puts behind everything; a click opens it in front. Sparkle documents the behind-other-apps behaviour for background apps, and on macOS 27 even its "immediate focus" case landed at window position 3 (measured). Tippi gets a notification delegate for the first time · **settings sync could recurse without end** when iCloud does not accept writes — re-entrancy guard, reproduced by a test with a deaf store |
