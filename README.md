...
- **Autocomplete while typing (Labs, experimental, off by default)** — pause briefly while typing and your **local** model suggests the next one to three words, in grey right at the cursor; **⇥ accepts**, any other key, Esc, a click or switching apps dismisses it. Tab is only intercepted while a suggestion is visible, so indenting in a code editor keeps working. Everything you type goes **only to the MLX server Tippi started itself** on this Mac (never a cloud provider, never a foreign server that happens to sit on the same port) and is **never stored or logged**. Password fields, secure input and an editable app list (password managers and terminals by default) are never read. Turn it on in Settings → General or from the menu bar. Design: `docs/SECURE-DESIGN-autocomplete.md`
