...
- **Notes window (v2.3)** — ⌥⌘N opens a resizable window: note list on the left, editor on the right. **Pin it (v2.4)** to float above every other app across switches/Spaces/full-screen. **AI titles (v2.5)**: a sparkles button inserts a short AI-generated title above your text, never replacing anything. Each note is its own plain `.txt` file, visible in Finder as **iCloud Drive → Tippi → Notes (v2.4.1)** — synced across your Macs via iCloud (local fallback when signed out). Pasting anything with formatting lands as clean text automatically, with a quiet toast confirming it — plus native spell check and a live word/character counter
