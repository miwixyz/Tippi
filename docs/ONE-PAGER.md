...
- **Notes window (v2.3)** — ⌥⌘N opens a resizable window: note list on the left, editor on the right. Each note is its own plain `.txt` file, synced across your Macs via iCloud (local fallback when signed out). Pasting anything with formatting lands as clean text automatically, with a quiet toast confirming it — plus native spell check and a live word/character counter
