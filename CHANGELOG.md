...
- **Windows looked like fog, not glass.** Settings, Notes and Welcome were `.clear` windows with a translucent material across their entire content. Any full-window translucency blurs the wallpaper down to its **average colour** — on a purple desktop the windows turned pink, with the wallpaper's pattern nowhere visible. Swapping `glassEffect` for `.regularMaterial` changed nothing, because the translucency itself was the problem, not the material providing it. This also contradicted the scope note in `GlassBackground.swift` ("nothing in the content layer uses it") and Apple's HIG, which puts Liquid Glass in the functional layer only. Window bodies are solid again, sidebars stay translucent — the split Finder, Mail and Notes.app use. Liquid Glass remains on the floating panels, popups, toasts and indicators. Reported on macOS 27; macOS 26 rendered the same code far less obtrusively, which is why it went unnoticed.
- **Notes window: seam under the title bar.** `titlebarAppearsTransparent` alone (tried in 2.8.x) only moved the edge down, because the toolbar paints its own opaque strip across the full window width. Title bar **and** toolbar background are now both transparent, so the window body runs uninterrupted.
...
- **The key for the single-key style is recorded, not picked from a list.** Click the field, press the key, done. A menu of eight names asked you to know which key under your fingers is called "Left Control" — and the first real test of it failed exactly that way: the setting read Left Control while the key being pressed was left Command, so the hot key did nothing and gave no hint why. Pressing the key cannot go wrong like that, and it works on remapped keyboards, which a fixed list never would.
...
- **Live suggestions while typing.** From `:e` onwards a short list of matching emoji appears next to the cursor. **Space** accepts the top match, a click picks any of them, typing more narrows the list, Escape dismisses it. Arrow keys and Return deliberately do nothing: this popup never takes keyboard focus, so everything you type keeps going to the app you're actually writing in. Space is the accept key precisely because it can be taken back — Tab moves focus to the next field in Mail and Slack, and Return would send the message.
...
### Fixed
- **The Translate Quick Panel clipped multi-line text and had no way to apply a result.** The input was a single-line field from when this panel was only ever typed into by hand; since v2.0.0 it pre-fills with your current selection, so multi-paragraph input is the normal case — and the overflow rendered outside the field's bounds, bleeding over the result below it. The panel height was also hard-coded, cutting off longer translations with no way to reach the rest. The input now grows to six lines, the height follows the content, the result scrolls, and the panel keeps its top edge anchored while growing instead of jumping upward.
- **Translate Quick Panel: new Replace button (⌘⏎)** writes the translation back over the text it came from, using the same Accessibility/clipboard ladder as the popup's local quick actions. It appears only when the panel was opened on an actual selection — with hand-typed input there is nothing to replace. This reverses the original "result is read-only" decision on purpose: the panel now starts from your selection, so writing back is the obvious next step.
