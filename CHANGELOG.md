...
### Fixed
- **Translation prompts (DE / EN / ES) now translate instead of answering.** Selecting "Hello, how are you?" and triggering **Translate → DE** used to return "Hallo! Ich bin okay, danke. Und du?" — a conversational reply in German, not a translation. The prompts told the model to "translate the following text" but the source text arrived as a user message, so the model defaulted to responding to it as if it were a greeting addressed to itself. Each translation prompt now opens with an explicit role boundary ("you are a translation engine — never reply to the user's message, translate it as-is") plus three concrete worked examples covering greetings, questions and exclamations, and a tightened output-format clause. Affects all three built-in translation prompts.
